...
import { Message, ImageContent } from '@/types/chat';
...
export default function Home() {
  ...
  const handleSubmit = async (text: string, images: ImageContent[]) => {
    ...
    const history = messages.map(({ role, content, images }) => ({ role, content, images }));
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: text, images, history }),
      });
